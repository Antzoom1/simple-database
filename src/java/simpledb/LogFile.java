...
public class LogFile {
    ...
    public void rollback(TransactionId tid)
        ...
        synchronized (Database.getBufferPool()) {
            synchronized (this) {
                preAppend();
                // some code goes here
                // base case for rollback (if there are no more log records)
                Long firstLogRecord = tidToFirstLogRecord.get(tid.getId());
                raf.seek(firstLogRecord);
                // Check that the first log record is a begin record
                if (raf.readInt() != BEGIN_RECORD || raf.readLong() != tid.getId()) {
                    throw new RuntimeException("First log record is not a begin record");
                }
                // Read the first log record
                raf.readLong();
                // Read the first log record's start location
                Set<PageId> undo = new HashSet<PageId>();
                // recursive case for rollback (if there are more log records)
                while (raf.length() > raf.getFilePointer()) {
                    // Check that the next log record is an update record
                    if (!(raf.readInt() == UPDATE_RECORD)) {
                        continue;
                    }
                    // Check that the next log record is for the same transaction
                    if (!(raf.readLong() == tid.getId())) {
                        continue;
                    }
                    // Read the next log record's start location
                    Page prior = readPageData(raf);
                    readPageData(raf);
                    // Write the page back to disk if it has not been written yet
                    if (!(undo.contains(prior.getId()))) {
                        Database.getCatalog().getDatabaseFile(prior.getId().getTableId()).writePage(prior);
                        // Clear the page from the buffer pool
                        Database.getBufferPool().discardPage(prior.getId());
                        undo.add(prior.getId());
                    }
                    // Read the next log record's start location
                    raf.readLong();
                }
    ...
    public void recover() throws IOException {
        synchronized (Database.getBufferPool()) {
            synchronized (this) {
                // initialize the log
                recoveryUndecided = false;
                Set<Long> commitTransactionCheckpoint = new HashSet<>();
                Map<Long, Long> activeTransactions = new HashMap<>();

                // Reads the last checkpoint base case
                raf.seek(0);
                long recentCheckpoint = raf.readLong();
                if (!(recentCheckpoint == NO_CHECKPOINT_ID)) {
                    raf.seek(recentCheckpoint);
                }
                ...
                while (raf.getFilePointer() < raf.length()) {
                    ...
                    switch (logRecordType) {
                        // Reads the begin record
                        case BEGIN_RECORD:
                            long transactionId = raf.readLong();
                            long recOffset = raf.readLong();
                            activeTransactions.put(transactionId, recOffset);
                            break;
                        // Reads the checkpoint record
                        case CHECKPOINT_RECORD:
                            raf.readLong();
                            long numberActiveTexts = raf.readInt();
                            for (int i = 0; i < numberActiveTexts; i++) {
                                long activeId = raf.readLong();
                                long activeOffset = raf.readLong();
                                activeTransactions.put(activeId, activeOffset);
                            }
                            raf.skipBytes(LONG_SIZE);
                            break;
                        // Reads the commit record
                        case COMMIT_RECORD:
                            long textId = raf.readLong();
                            raf.readLong();
                            commitTransactionCheckpoint.add(textId);
                            break;
                        // Reads the update record
                        case UPDATE_RECORD:
                            raf.readLong();
                            readPageData(raf);
                            readPageData(raf);
                            raf.readLong();
                            break;
                        // Reads the abort record
                        case ABORT_RECORD:
                            raf.skipBytes(LONG_SIZE * 2);
                            break;
                        // Reads the end record
                        default:
                            throw new RuntimeException("Log not supported in recovery");
                    }
                }

                for (Map.Entry<Long, Long> logEntry : activeTransactions.entrySet()) {
                    Long transactionId = logEntry.getKey();
                    Long startOffset = logEntry.getValue();
                    // If the transaction is committed, then write the page back to disk
                    if (commitTransactionCheckpoint.contains(transactionId)) {
                        handleUpdatedTransactions(startOffset, transactionId, true);
                    } else {
                        // If the transaction is not committed, then write the page back to disk but not committed
                        handleUpdatedTransactions(startOffset, transactionId, false);
                    }
                }
                // Clear the active transactions
    ...
    /**
     * Write a begin record to the log for the specified transaction.
     * 
     * @param tid The transaction being started.
     */
    private void handleUpdatedTransactions(long beginOffset, long transactionId, boolean committed) throws IOException {
        ...
        while (true) {
            try {
                ...
                if (logRecordType == UPDATE_RECORD) {
                    long recordTextId = raf.readLong();
                    Page beforePage = readPageData(raf);
                    Page afterPage = readPageData(raf);
                    raf.readLong();
                    // If the transaction is committed, then write the page back to disk
                    if (recordTextId == transactionId) {
                        // Write the page back to disk if it has not been written yet
                        Database.getCatalog().getDatabaseFile(beforePage.getId().getTableId()).writePage(
                                committed ? afterPage : beforePage);
                    }
                } else {
                    // If the next log record is not an update record, then break
                    raf.skipBytes(LONG_SIZE * 2);
                }
            } catch (EOFException e) {
                break;
